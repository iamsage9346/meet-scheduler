...
export default function BookingSlots({
  ...
  const isSlotInPast = (slot: string) => {
    const now = new Date();
    const slotDate = new Date(slot.replace('T', ' ') + ':00');
    return slotDate < now;
  };

  // Filter and sort slots, excluding past slots for non-hosts
  const slotsByDate = useMemo(() => {
    const grouped = new Map<string, string[]>();
    hostSlots.forEach((slot) => {
      // Skip past slots (unless viewing as host)
      if (!isHost && isSlotInPast(slot)) return;
    ...
    grouped.forEach((slots, date) => {
      if (slots.length === 0) grouped.delete(date);
    });
    return grouped;
  }, [hostSlots, isHost]);
